...
/* --- should work now, no warning needed anymore :) ---dante
...
*/
	 ...
	 kdfread(&sprite[0],sizeof(spritetype),MAXSPRITES,fil);
		 ...
		 kdfread16(&headspritesect[0],MAXSECTORS+1,fil);
	 kdfread16(&prevspritesect[0],MAXSPRITES,fil);
	 kdfread16(&nextspritesect[0],MAXSPRITES,fil);
	 kdfread16(&headspritestat[0],MAXSTATUS+1,fil);
	 kdfread16(&prevspritestat[0],MAXSPRITES,fil);
	 kdfread16(&nextspritestat[0],MAXSPRITES,fil);
	 kdfread16(&numcyclers,1,fil);
	 kdfread16(&cyclers[0][0],MAXCYCLERS*6,fil);
     
	 kdfread(ps,sizeof(ps),1,fil);
     kdfread(po,sizeof(po),1,fil);
	 
	 kdfread16(&numanimwalls,1,fil);
     ...
     kdfread16((short *)&spriteqloc,1,fil);
     kdfread16((short *)&spriteqamount,1,fil);
     kdfread16((short *)&spriteq[0],spriteqamount,fil);
	 kdfread16(&mirrorcnt,1,fil);
	 kdfread16(&mirrorwall[0],64,fil);
		 kdfread16(&mirrorsector[0],64,fil);
     ...
     kdfread16(&numclouds,1,fil);
     kdfread16(&clouds[0],128,fil);
     kdfread16(&cloudx[0],128,fil);
     kdfread16(&cloudy[0],128,fil);

     kdfread(&scriptptrs[0],1,MAXSCRIPTSIZE,fil);
     kdfread32(&script[0],MAXSCRIPTSIZE,fil);
	...
	if( scriptptrs[i] )
     ...
     kdfread32(&actorscrptr[0],MAXTILES,fil);
	...
	if( scriptptrs[i]&4 ) T6 += j;
	 ...
	 kdfread32(&lockclock,1,fil);
     kdfread16(&pskybits,1,fil);
     kdfread16(&pskyoff[0],MAXPSKYTILES,fil);

	 kdfread32(&animatecnt,1,fil);
	 kdfread16(&animatesect[0],MAXANIMATES,fil);
	 kdfread32(&animateptr[0],MAXANIMATES,fil);
     for(i = animatecnt-1;i>=0;i--) animateptr[i] = (long *)((long)animateptr[i]+(long)(&sector[0]));
	 kdfread32(&animategoal[0],MAXANIMATES,fil);
	 kdfread32(&animatevel[0],MAXANIMATES,fil);

	 kdfread(&earthquaketime,sizeof(earthquaketime),1,fil);
     kdfread16(&ud.from_bonus,1,fil);
     kdfread16(&ud.secretlevel,1,fil);
     kdfread32(&ud.respawn_monsters,1,fil);
     ud.m_respawn_monsters = ud.respawn_monsters;
     kdfread32(&ud.respawn_items,1,fil);
     ud.m_respawn_items = ud.respawn_items;
     kdfread32(&ud.respawn_inventory,1,fil);
     ud.m_respawn_inventory = ud.respawn_inventory;

     kdfread(&ud.god,sizeof(ud.god),1,fil);
     kdfread32(&ud.auto_run,1,fil);
     kdfread32(&ud.crosshair,1,fil);
     kdfread32(&ud.monsters_off,1,fil);
     ud.m_monsters_off = ud.monsters_off;
     kdfread16(&ud.last_level,1,fil);
     kdfread(&ud.eog,sizeof(ud.eog),1,fil);

     kdfread32(&ud.coop,1,fil);
     ud.m_coop = ud.coop;
     kdfread32(&ud.marker,1,fil);
     ud.m_marker = ud.marker;
     kdfread32(&ud.ffire,1,fil);
     ud.m_ffire = ud.ffire;

     kdfread16(&camsprite,1,fil);
     kdfread16(&connecthead,1,fil);
     kdfread16(connectpoint2,MAXPLAYERS,fil);
     kdfread(&numplayersprites,sizeof(numplayersprites),1,fil);
     kdfread16((short *)&frags[0][0],MAXPLAYERS*MAXPLAYERS,fil);

     kdfread32(&randomseed,1,fil);
     kdfread16(&global_random,1,fil);
     kdfread32(&parallaxyscale,1,fil);
...
int saveplayer(signed char spot)
{
	...
	fnptr = fullname;
     ...
     dfwrite32(&bv,1,fil);
     dfwrite32(&ud.multimode,1,fil);

	 dfwrite(&ud.savegame[spot][0],19,1,fil);
	 dfwrite32(&ud.volume_number,1,fil);
     dfwrite32(&ud.level_number,1,fil);
	 dfwrite32(&ud.player_skill,1,fil);
     dfwrite((char *)waloff[MAXTILES-1],160,100,fil);

	 dfwrite16(&numwalls,1,fil);
     dfwrite(&wall[0],sizeof(walltype),MAXWALLS,fil);
	 dfwrite16(&numsectors,1,fil);
     dfwrite(&sector[0],sizeof(sectortype),MAXSECTORS,fil);
	 dfwrite(&sprite[0],sizeof(spritetype),MAXSPRITES,fil);
	 
		 dfwrite16(&headspritesect[0],MAXSECTORS+1,fil);
	 dfwrite16(&prevspritesect[0],MAXSPRITES,fil);
	 dfwrite16(&nextspritesect[0],MAXSPRITES,fil);
	 dfwrite16(&headspritestat[0],MAXSTATUS+1,fil);
	 dfwrite16(&prevspritestat[0],MAXSPRITES,fil);
	 dfwrite16(&nextspritestat[0],MAXSPRITES,fil);
	 dfwrite16(&numcyclers,1,fil);
	 dfwrite16(&cyclers[0][0],MAXCYCLERS*6,fil);
     
	 dfwrite(ps,sizeof(ps),1,fil);
     dfwrite(po,sizeof(po),1,fil);
	 
	 dfwrite16(&numanimwalls,1,fil);
     ...
     dfwrite16(&spriteqloc,1,fil);
     dfwrite16(&spriteqamount,1,fil);
     dfwrite16(&spriteq[0],spriteqamount,fil);
	 dfwrite16(&mirrorcnt,1,fil);
	 dfwrite16(&mirrorwall[0],64,fil);
	 dfwrite16(&mirrorsector[0],64,fil);
     ...
     dfwrite16(&numclouds,1,fil);
     dfwrite16(&clouds[0],128,fil);
     dfwrite16(&cloudx[0],128,fil);
     dfwrite16(&cloudy[0],128,fil);
     ...
     dfwrite32(&script[0],MAXSCRIPTSIZE,fil);
	...
	actorscrptr[i] = (long *)j;
     ...
     dfwrite32(&actorscrptr[0],MAXTILES,fil);
     for(i=0;i<MAXTILES;i++)
	 if(actorscrptr[i])
     {
	 j = (long)actorscrptr[i]+(long)&script[0];
	...
	if(scriptptrs[i]&4)
	 ...
	 dfwrite32(&lockclock,1,fil);
     dfwrite16(&pskybits,1,fil);
     dfwrite16(&pskyoff[0],MAXPSKYTILES,fil);
	 
	 dfwrite32(&animatecnt,1,fil);
	 dfwrite16(&animatesect[0],MAXANIMATES,fil);
	 for(i = animatecnt-1;i>=0;i--) animateptr[i] = (long *)((long)animateptr[i]-(long)(&sector[0]));
	 dfwrite32(&animateptr[0],MAXANIMATES,fil);
	 for(i = animatecnt-1;i>=0;i--) animateptr[i] = (long *)((long)animateptr[i]+(long)(&sector[0]));
	 dfwrite32(&animategoal[0],MAXANIMATES,fil);
	 dfwrite32(&animatevel[0],MAXANIMATES,fil);

	 dfwrite(&earthquaketime,sizeof(earthquaketime),1,fil);
	 dfwrite16(&ud.from_bonus,1,fil);
     dfwrite16(&ud.secretlevel,1,fil);
     dfwrite32(&ud.respawn_monsters,1,fil);
     dfwrite32(&ud.respawn_items,1,fil);
     dfwrite32(&ud.respawn_inventory,1,fil);

     dfwrite(&ud.god,sizeof(ud.god),1,fil);
     dfwrite32(&ud.auto_run,1,fil);
     dfwrite32(&ud.crosshair,1,fil);
     dfwrite32(&ud.monsters_off,1,fil);
     dfwrite16(&ud.last_level,1,fil);
     dfwrite(&ud.eog,sizeof(ud.eog),1,fil);
	 
     dfwrite32(&ud.coop,1,fil);
     dfwrite32(&ud.marker,1,fil);
     dfwrite32(&ud.ffire,1,fil);
	 
     dfwrite16(&camsprite,1,fil);
     dfwrite16(&connecthead,1,fil);
     dfwrite16(connectpoint2,MAXPLAYERS,fil);
     dfwrite(&numplayersprites,sizeof(numplayersprites),1,fil);
     dfwrite16((short *)&frags[0][0],MAXPLAYERS*MAXPLAYERS,fil);

     dfwrite32(&randomseed,1,fil);
     dfwrite16(&global_random,1,fil);
     dfwrite32(&parallaxyscale,1,fil);
